...
export function GeneralRanking({ season, round }: { season: number; round: number; marketOpen?: boolean }) {
  const state = useGeneralRanking(season, round);
  return <GeneralRankingView season={season} round={state.ranking?.rodada ?? round} data={state.ranking} loading={state.loading} error={state.error} warning={state.warning} retry={state.retry} retrying={state.retrying} firstRound={state.firstRound} />;
}

export function GeneralRankingView({ season, round, data, loading, error, warning = null, retry, retrying = false, firstRound = false }: { season?: number; round: number; data: GeneralRankingResponse | null; loading: boolean; error: boolean; warning?: RankingWarning; retry?: () => void; retrying?: boolean; firstRound?: boolean }) {
