...
export function JogosHoje({ embedded = false }: { embedded?: boolean }) {
  ...
  const date = data?.data ? new Date(`${data.data}T12:00:00Z`) : null;
  const dateLabel = date && !Number.isNaN(date.getTime()) ? date.toLocaleDateString("pt-BR", { day: "numeric", month: "long", timeZone: "America/Sao_Paulo" }) : null;
  return <section className={`${embedded ? "" : shared.card} ${styles.section}`} aria-label="Jogos de hoje">
    <header>{embedded ? <span className={styles.date}>Hoje{dateLabel ? ` • ${dateLabel}` : ""}</span> : <h2>Jogos de hoje</h2>}<div className={shared.headingActions}>
      ...
      {!embedded && <Link className={shared.viewAll} href="/jogos">Ver todos <ArrowRight size={14} /></Link>}
    </div></header>
