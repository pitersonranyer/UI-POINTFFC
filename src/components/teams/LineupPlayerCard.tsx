...
export interface LineupPlayerCardProps {
  player: CartolaTeamLineupAthlete; clubs: Record<string, CartolaClub>; isCaptain?: boolean; isReserve?: boolean; isLuxuryReserve?: boolean; compact?: boolean;
}

export function LineupPlayerCard({ player, clubs, isCaptain = false, isReserve = false, isLuxuryReserve = false, compact = false }: LineupPlayerCardProps) {
  ...
  return <article className={`${styles.playerCard} ${compact ? styles.compactPlayer : ""}`} data-reserve={isReserve || undefined}>
    <span className={styles.playerPhoto}>{photo ? <img src={photo} alt="" /> : <ImageOff />}{isCaptain && <b className={styles.captain} title="Capitão">C</b>}</span>
