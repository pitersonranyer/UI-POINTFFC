...
export function TeamDetailPage({ timeId, rodada }: { timeId: number; rodada?: number }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [nextDashboard, nextTeam] = await Promise.all([buscarDashboard(), rodada === undefined ? buscarEscalacaoTime(timeId) : buscarEscalacaoTime(timeId, rodada)]);
      const receivedRound = nextTeam.time.rodada_time_id ?? nextTeam.rodada_atual;
      if (rodada !== undefined && receivedRound !== undefined && receivedRound !== rodada) throw new Error("Escalação de outra rodada.");
      const round = rodada ?? receivedRound ?? rodadaDeEscalacao(nextDashboard.rodada, nextDashboard.mercadoAberto);
  ...
  const expectedRound = rodadaDeEscalacao(dashboard.rodada, dashboard.mercadoAberto), lineupRound = rodada ?? team.time.rodada_time_id ?? team.rodada_atual ?? expectedRound;
  ...
  return <main className={styles.shell}><Link href="/" className={styles.back}><ArrowLeft />Voltar ao Ranking Geral</Link><header className={styles.teamHeader}>{team.time.url_escudo_png ? <img src={team.time.url_escudo_png} alt={`Escudo do ${team.time.nome}`} /> : <span>{team.time.nome.slice(0, 2).toUpperCase()}</span>}<div><p>Detalhe do time</p><h1>{team.time.nome}</h1>{team.time.nome_cartola && <small>{team.time.nome_cartola}</small>}</div><dl>
    ...
    <section className={styles.lineupHeader}><div><p className="eyebrow">Escalação utilizada</p><h2>Escalação da rodada {lineupRound}</h2><small>{rodada !== undefined ? `Exibindo a escalação da rodada ${rodada}.` : dashboard.mercadoAberto ? `O mercado está aberto. Exibindo a escalação da rodada anterior.` : `Mercado fechado. Exibindo a escalação da rodada atual.`}</small>{formation && <b>Esquema {formation}</b>}</div><div className={styles.toggle} aria-label="Visualização da escalação"><button type="button" className={view === "field" ? styles.active : ""} onClick={() => changeView("field")}><LayoutGrid />Campo</button><button type="button" className={view === "list" ? styles.active : ""} onClick={() => changeView("list")}><List />Lista</button></div></section>
