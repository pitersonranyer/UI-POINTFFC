...
export function PointCompetitionPage({ id }: { id: number }) {
  ...
  const [importing, setImporting] = useState(false), [importText, setImportText] = useState(""), [importPreview, setImportPreview] = useState<CartolaTeam[] | null>(null), [selectedImportIds, setSelectedImportIds] = useState<Set<number>>(new Set()), [importNotFound, setImportNotFound] = useState<number[]>([]), [importRetry, setImportRetry] = useState<number[]>([]);
  ...
  const resetEnrollmentModal = () => { setSelected(new Set()); setTeamQuery(""); setImporting(false); setImportText(""); setImportPreview(null); setSelectedImportIds(new Set()); setImportNotFound([]); setImportRetry([]); };
  ...
  const toggleImportTeam = (timeId: number) => setSelectedImportIds((current) => { const next = new Set(current); if (next.has(timeId)) next.delete(timeId); else next.add(timeId); return next; });
  const findImport = async () => { const ids = normalizeImportIds(importText); if (running.current || !ids) return; running.current = true; setBusy(true); setSectionError(""); setSelectedImportIds(new Set()); try { const result = await teamService.buscarTimesPorIds(ids); setImportPreview(result.times); setImportNotFound(result.naoEncontrados); setImportRetry(result.tentarNovamente); } catch (e) { setSectionError(message(e)); } finally { running.current = false; setBusy(false); } };
  const importTeams = async () => { const selectedTeams = importPreview?.filter((team) => selectedImportIds.has(team.timeId)) ?? []; if (running.current || !selectedTeams.length) return; running.current = true; setBusy(true); setSectionError(""); try { await teamService.importarMeusTimes(selectedTeams); await refreshTeams(); setTeamQuery(""); setImporting(false); setImportText(""); setImportPreview(null); setSelectedImportIds(new Set()); setFeedback("Times adicionados aos Meus Times. Selecione-os para inscrever."); } catch (e) { setSectionError(message(e)); } finally { running.current = false; setBusy(false); } };
    ...
    {loading || authLoading ? <div className={styles.skeleton} role="status" aria-label="Carregando competição"><div className={styles.skeletonHero} /><div className={styles.skeletonTabs} /><div className={styles.skeletonPanel} /></div>
      ...
      : summary && competition && <>
        ...
        {modal && <Dialog title="Inscrever times" close={() => { if (!busy) { setModal(false); resetEnrollmentModal(); } }} wide busy={busy}>
          {importing ? <>
            <button type="button" className={styles.backToTeams} onClick={() => { setImporting(false); setImportPreview(null); setSelectedImportIds(new Set()); }} disabled={busy}>← Meus times</button>
            ...
            {!importPreview ? <textarea aria-label="IDs dos times" className={styles.importInput} value={importText} disabled={busy} onChange={(event) => setImportText(event.target.value)} placeholder="44566162;30157334;13933388" /> : <><div className={styles.importSelectionTools}><strong>{importPreview.length} {importPreview.length === 1 ? "encontrado" : "encontrados"} • {selectedImportIds.size} {selectedImportIds.size === 1 ? "selecionado" : "selecionados"}</strong><span><button type="button" onClick={() => setSelectedImportIds(new Set(importPreview.map((team) => team.timeId)))} disabled={busy || selectedImportIds.size === importPreview.length}>Selecionar todos</button><button type="button" onClick={() => setSelectedImportIds(new Set())} disabled={busy || !selectedImportIds.size}>Desmarcar todos</button></span></div><div className={styles.importPreview}>{importPreview.map((team) => <label key={team.timeId} className={selectedImportIds.has(team.timeId) ? styles.importSelected : ""}><input type="checkbox" checked={selectedImportIds.has(team.timeId)} disabled={busy} onChange={() => toggleImportTeam(team.timeId)} aria-label={`Selecionar ${team.nome} para importar`} />{team.escudoUrl ? <img src={team.escudoUrl} alt="" /> : <span className={styles.choiceShield}>{team.nome.slice(0, 2).toUpperCase()}</span>}<span><b>{team.nome}</b><small>{team.nomeCartoleiro} · ID {team.timeId}</small></span></label>)}</div></>}
            ...
            <div className={styles.actions}><button type="button" onClick={() => { setImporting(false); setImportPreview(null); setSelectedImportIds(new Set()); }} disabled={busy}>Cancelar</button>{!importPreview ? <button type="button" className={styles.primary} onClick={() => void findImport()} disabled={busy || !normalizeImportIds(importText)}>{busy ? <Loader2 className={styles.spin} /> : <Search />}Buscar times</button> : <button type="button" className={styles.primary} onClick={() => void importTeams()} disabled={busy || !selectedImportIds.size}>{busy ? <Loader2 className={styles.spin} /> : <Download />}{selectedImportIds.size === 1 ? "Importar 1 time" : selectedImportIds.size > 1 ? `Importar ${selectedImportIds.size} times` : "Importar times"}</button>}</div>
          </> : <>
            ...
            {teams.length ? <div className={styles.choices}>{filteredTeams.map((team) => { const registered = registeredIds.has(team.timeId); const limitReached = !selected.has(team.timeId) && selected.size >= remaining; return <label key={team.timeId} className={`${registered ? styles.registered : ""} ${selected.has(team.timeId) ? styles.choiceSelected : ""}`}><input type="checkbox" checked={selected.has(team.timeId)} onChange={() => toggleTeam(team.timeId)} disabled={busy || registered || limitReached} aria-label={registered ? `${team.nome}: Já inscrito` : `Selecionar ${team.nome}`} />{team.escudoUrl ? <img src={team.escudoUrl} alt="" /> : <span className={styles.choiceShield}>{team.nome.slice(0, 2).toUpperCase()}</span>}<span><strong>{team.nome}</strong><small>{registered ? "Já inscrito" : team.nomeCartoleiro}</small></span></label>; })}{!filteredTeams.length && <p className={styles.noTeams}>Nenhum time encontrado.</p>}</div> : <p>Nenhum time vinculado. Importe seus times do Cartola para continuar.</p>}
            {sectionError && <p role="alert" className={styles.inlineError}>{sectionError}</p>}
            <div className={styles.enrollmentFooter}><div><strong>{selected.size} {selected.size === 1 ? "time selecionado" : "times selecionados"}</strong><small>{Number.isFinite(remaining) ? `${Math.max(0, remaining - selected.size)} ${remaining - selected.size === 1 ? "vaga restante" : "vagas restantes"}` : "Sem limite de vagas"}</small></div><button type="button" className={styles.primary} disabled={busy || !selected.size || selected.size > remaining} onClick={() => void enroll()}>{busy ? <><Loader2 className={styles.spin} />Inscrevendo...</> : selected.size === 1 ? "Inscrever time" : `Inscrever ${selected.size} times`}</button></div>
          </>}
        </Dialog>}
      </>}
  </main>;
