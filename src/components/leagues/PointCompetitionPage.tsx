...
type Tab = "Visão geral" | "Meus times" | "Participantes" | "Ranking" | "Premiação";
const tabs: Tab[] = ["Visão geral", "Meus times", "Participantes", "Ranking", "Premiação"];
const tabIcons = { "Visão geral": Home, "Meus times": Shield, Participantes: Users, Ranking: Trophy, "Premiação": Gift };
...
function Entries({ entries, ownIds = new Set<number>(), ranking = false }: { entries: (Entry | RankingEntry)[]; ownIds?: Set<number>; ranking?: boolean }) {
  ...
  return <div className={styles.entries}>{entries.map((entry) => { const own = ownIds.has(entry.id) || ("inscricaoId" in entry && ownIds.has(entry.inscricaoId)); const movement = entry.posicao !== null && entry.posicaoAnterior !== null ? entry.posicaoAnterior - entry.posicao : null; return <article className={`${styles.entry} ${own ? styles.own : ""}`} key={entry.id ?? (entry as RankingEntry).inscricaoId}><strong className={styles.position}>{position(entry.posicao)}</strong>{ranking && <small className={styles.movement}>{movement === null || movement === 0 ? "—" : movement > 0 ? `↑ ${movement}` : `↓ ${-movement}`}</small>}{entry.escudoUrl && <img src={entry.escudoUrl} alt="" />}<div className={styles.teamName}><strong>{entry.nomeTime}{own && " · Meu time"}</strong><small>{entry.nomeCartoleiro ?? "Cartoleiro não informado"}</small></div><span className={styles.score}>{score(entry.pontuacao)}</span>{!ranking && <small className={styles.previous}>Anterior: {position(entry.posicaoAnterior)}</small>}</article>; })}</div>;
}
      ...
      : summary && competition && <>
        ...
        {tab === "Visão geral" && <div className={styles.overview}>
          ...
          <section className={styles.rankingPreview}><div className={styles.heading}><div><h2>Classificação parcial</h2><p>Acompanhe a disputa da rodada.</p></div><button type="button" onClick={() => setTab("Ranking")}>Ver ranking completo <ArrowRight size={15} aria-hidden="true" /></button></div>
            {rankingLoaded && ranking.some((item) => item.pontuacao !== null) ? <Entries entries={ranking.slice(0, 5)} ownIds={ownIds} ranking /> : <p className={styles.previewEmpty}>{rankingLoaded ? "Ainda não há pontuações nesta rodada." : "Abra o ranking para acompanhar a classificação."}</p>}
          </section>
        </div>}
        ...
        {sectionLoading && ["Meus times", "Participantes", "Ranking"].includes(tab) ? <p role="status" className={styles.sectionLoading}>Carregando...</p> : tab === "Meus times" && isAuthenticated ? <section className={styles.panel}><div className={styles.panelTitle}><Shield aria-hidden="true" /><h2>Meus times</h2></div>{entries.length ? <Entries entries={entries} /> : <p className={styles.empty}>Você ainda não inscreveu times nesta competição.</p>}</section> : tab === "Participantes" ? <section className={styles.panel}><div className={styles.panelTitle}><Users aria-hidden="true" /><h2>Participantes</h2></div><Entries entries={entries} /></section> : tab === "Ranking" ? <section className={styles.panel}><div className={styles.heading}><div><h2>Ranking</h2>{roundLabel !== null && roundLabel !== undefined && <p>Rodada {roundLabel}</p>}</div><button type="button" onClick={() => void loadTab("Ranking")}><RefreshCw size={15} aria-hidden="true" />Atualizar</button></div><Entries entries={ranking} ownIds={ownIds} ranking /></section> : null}
