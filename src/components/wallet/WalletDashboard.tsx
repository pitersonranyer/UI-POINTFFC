...
import { ArrowDownLeft, ArrowUpRight, Check, Copy, Plus, RotateCcw, WalletCards, X } from "lucide-react";
...
const dateLabel = (date: string) => {
  ...
  if (value.toDateString() === today.toDateString()) return `Hoje, ${value.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}`;
  return value.toLocaleDateString("pt-BR");
};

export function WalletDashboard() {
  const { wallet, transactions, activeCharge, isLoading, error, createPixDeposit, simulatePixPayment, clearActiveCharge, resetMock } = useWallet();
  const [modalOpen, setModalOpen] = useState(false); const [value, setValue] = useState(""); const [formError, setFormError] = useState("");
  const [working, setWorking] = useState(false); const [qrCode, setQrCode] = useState(""); const [copied, setCopied] = useState(false); const [showAll, setShowAll] = useState(false); const [success, setSuccess] = useState("");
  const totals = useMemo(() => transactions.reduce((sum, item) => item.status === "CONFIRMADO" ? { income: sum.income + (item.amount > 0 ? item.amount : 0), outcome: sum.outcome + (item.amount < 0 ? Math.abs(item.amount) : 0) } : sum, { income: 0, outcome: 0 }), [transactions]);
  ...
  return <div className="page-shell">
    ...
    <section className={styles.balanceCard} aria-label="Resumo da carteira"><div className={styles.balanceTop}><span className={styles.walletIcon}><WalletCards /></span><div><small>Saldo disponível</small><strong>{formatCurrency(wallet?.balance ?? 0)}</strong></div></div><button type="button" onClick={openModal}><Plus size={19} />Adicionar saldo</button><div className={styles.summary}><div><ArrowDownLeft /><span>Entradas no período<strong>{formatCurrency(totals.income)}</strong></span></div><div><ArrowUpRight /><span>Saídas no período<strong>{formatCurrency(totals.outcome)}</strong></span></div></div></section>
    <section className={styles.statement}><div className={styles.sectionHeading}><div><p className="eyebrow">Extrato</p><h2>Últimas movimentações</h2></div>{transactions.length > 4 && <button type="button" onClick={() => setShowAll((current) => !current)}>{showAll ? "Ver recentes" : "Ver todas"}</button>}</div>
      {transactions.length === 0 ? <div className={styles.empty}>Nenhuma movimentação encontrada.</div> : <div className={styles.transactions}>{(showAll ? transactions : transactions.slice(0, 4)).map((transaction) => <TransactionRow key={transaction.id} transaction={transaction} />)}</div>}
    </section>
...
function TransactionRow({ transaction }: { transaction: WalletTransaction }) {
  ...
  return <article className={styles.transaction}><span className={incoming ? styles.incomingIcon : styles.outgoingIcon}>{incoming ? <ArrowDownLeft /> : <ArrowUpRight />}</span><div className={styles.transactionInfo}><strong>{transaction.description}</strong><small>{dateLabel(transaction.createdAt)} · {transaction.status.charAt(0) + transaction.status.slice(1).toLowerCase()}</small></div><strong className={incoming ? styles.incoming : styles.outgoing}>{incoming ? "+" : "−"} {formatCurrency(Math.abs(transaction.amount))}</strong></article>;
}
