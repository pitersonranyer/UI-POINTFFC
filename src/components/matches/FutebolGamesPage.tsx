...
export function FutebolGamesPage({ initialCodigo }: { initialCodigo?: FutebolCompeticaoCodigo }) {
  const [today, setToday] = useState(initialCodigo == null);
  const [codigo, setCodigo] = useState<FutebolCompeticaoCodigo>(initialCodigo ?? "BSA");
  const { data, loading, error } = useFutebolRodada(!today, codigo);
  ...
  return <div className={styles.shell}>
    ...
    <header className={styles.header}>
      ...
      <div className={styles.headerMark} aria-hidden="true">{today ? <><CalendarDays size={40} /><span>HOJE</span></> : <><CompetitionLogo key={codigo} codigo={codigo} nome={competicao.nome} url={competicao.logoUrl} /><span>{competicao.nome.toUpperCase()}</span></>}</div>
    </header>

    <nav className={styles.competitionSelector} aria-label="Selecionar competição"><button type="button" className={today ? styles.selectedCompetition : undefined} aria-pressed={today} onClick={() => { setToday(true); window.history.replaceState(null, "", "/jogos"); }}>Hoje</button>{futebolCompeticoes.map(item => <button key={item.codigo} type="button" className={!today && item.codigo === codigo ? styles.selectedCompetition : undefined} aria-pressed={!today && item.codigo === codigo} onClick={() => { setToday(false); setCodigo(item.codigo); window.history.replaceState(null, "", `/jogos?competicao=${item.codigo}`); }}>{item.aba}</button>)}</nav>
...
function TodayGames() {
  ...
  const date = data ? localDate(`${data.data}T12:00:00Z`) : null;
  const dateLabel = date?.toLocaleDateString("pt-BR", { day: "numeric", month: "long", timeZone: "America/Sao_Paulo" });
  const games = data?.total ? data.jogos : [];
  const liveCount = games.filter(game => game.status === "IN_PLAY").length;
  return <>
    <section className={`${styles.competition} ${styles.todayCompetition}`} aria-label="Jogos de hoje">
      <div className={styles.competitionIdentity}><span className={styles.competitionIcon}><CalendarDays size={28} aria-hidden="true" /></span><div><small>HOJE</small><strong>Jogos de hoje</strong></div></div>
      <div className={styles.round}><span>{dateLabel}</span></div>
    </section>
    <div className={styles.agendaHeading}><div><CalendarDays size={16} /><h2>Agenda de jogos</h2></div>{liveCount > 0 ? <span className={styles.liveSummary}><i />{liveCount} {liveCount === 1 ? "jogo ao vivo" : "jogos ao vivo"}</span> : data && games.length > 0 ? <span>{games.length} confrontos</span> : null}</div>
    {loading ? <div role="status" aria-label="Carregando jogos de hoje" className={styles.loading}><span>Preparando os jogos de hoje...</span><div className={styles.grid}>{[0, 1, 2].map(item => <div className={styles.skeleton} key={item}><i /><div><i /><i /></div><i /></div>)}</div></div>
      : error ? <section className={styles.feedback} role="alert"><CalendarDays size={28} /><h2>Os jogos voltam em instantes</h2><p>Não foi possível carregar os jogos de hoje.</p><button type="button" onClick={atualizar}>Tentar novamente</button></section>
      : !games.length ? <section className={styles.feedback}><CalendarDays size={28} /><h2>Agenda de hoje</h2><p>Nenhum jogo programado para hoje.</p></section>
      : <div className={styles.grid}>{games.map(game => <GameCard key={game.id} game={game} codigo={game.competicao.codigo} competitionName={game.competicao.nome} timeZone="America/Sao_Paulo" />)}</div>}
  </>;
}
...
function GameCard({ game, codigo, competitionName, timeZone }: { game: FutebolJogo; codigo: string; competitionName?: string; timeZone?: string }) {
  ...
  return <Link href={`/jogos?futebol=${game.id}${codigo === "BSA" ? "" : `&competicao=${codigo}`}`} className={`${styles.game} ${live ? styles.liveGame : ""}`} aria-label={`Ver confronto: ${game.mandante.nome} contra ${game.visitante.nome}`}>
    <div className={styles.schedule}><time dateTime={game.dataHoraUtc}>{date ? date.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit", ...(timeZone ? { timeZone } : {}) }) : "--:--"}</time><span className={`${styles.status} ${live ? styles.liveStatus : ""}`}>{live && <i />}{future ? "Em breve" : labels[game.status] ?? "A definir"}</span></div>
    ...
    <div className={styles.venue}>{competitionName ? <span>{competitionName}{venue ? ` · ${venue}` : ""}</span> : venue && <><MapPin size={14} aria-hidden="true" /><span>{venue}</span></>}</div>
