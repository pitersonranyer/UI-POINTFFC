...
export async function apiFetch<T>(path:string,options:ApiOptions={},retried=false):Promise<T>{
  ...
  if(authenticated){const token=sessionStorage.getItem("fantasy.accessToken");if(token)headers.set("Authorization",`Bearer ${token}`)}
  ...
  if(!response.ok){
    if(authenticated&&response.status===401&&!retried&&refreshSession){const token=await refreshSession();if(token)return apiFetch<T>(path,options,true)}
    const error=new ApiError(response.status,await message(response));
    if(authenticated&&(response.status===401||response.status===403))window.dispatchEvent(new CustomEvent("fantasy:session-invalid",{detail:{status:response.status}}));
    throw error
  }
